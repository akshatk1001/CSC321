...
def check_hashes():
    ...
    for user, hash in HASHES.items():
        start = time.time()
        print(f"Cracking hash for {user}: {hash}")
        for word in wordlist:
            if bcrypt.checkpw(word.encode('utf-8'), hash.encode('utf-8')):
                end = time.time()
                total = end - start
                print(f"Found password for {hash}: {word} in {total} seconds")
                answers.append((user, hash, word, total))
                break
